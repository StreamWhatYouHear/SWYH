...
namespace OpenSource.UPnP.AV.CdsMetadata
{
	...
	/// </summary>
	[Serializable()]
	public sealed class PropertyInt : ICdsElement, IToXmlData, IDeserializationCallback
	{
		/// <summary>
		...
		/// <summary>
		/// Attempts to case compareToThis as an int
		/// to do the comparison. If the cast
		/// fails, then it compares the current
		/// value against int.MinValue.
		/// </summary>
		...
		public int CompareTo (object compareToThis)
		{
			...
			int compareTo = int.MinValue;
			
			try
			{
				if (type == typeof(PropertyInt))
				{
					compareTo = ((PropertyInt)compareToThis).m_value;
				}
				else if (type == typeof(string))
				{
					compareTo = int.Parse(compareToThis.ToString());
				}
				else
				{
					compareTo = (int) compareToThis;
				}
			}
			catch
			{
		...
		public PropertyInt(XmlElement element)
		{

			this.m_value = int.Parse(element.InnerText);
		...
		public PropertyInt (string namespace_tag, int val)
		{
		...
		public PropertyInt(XmlReader reader)
		{

			this.m_value = XmlConvert.ToInt32(reader.Value);
		...
		/// </summary>
		public int _Value { get { return this.m_value; } }
