...
namespace OpenSource.UPnP.AV.CdsMetadata
{
	...
	/// simple long values without any attributes.
	/// </summary>
	[Serializable()]
	public sealed class PropertyLong : ICdsElement, IToXmlData, IDeserializationCallback
	{
		/// <summary>
		...
		/// <summary>
		/// Attempts to case compareToThis as an long
		/// to do the comparison. If the cast
		/// fails, then it compares the current
		/// value against long.MinValue.
		/// </summary>
		...
		public int CompareTo (object compareToThis)
		{
			...
			long compareTo = long.MinValue;
			
			try
			{
				if (type == typeof(PropertyLong))
				{
					compareTo = ((PropertyLong)compareToThis).m_value;
				}
				else if (type == typeof(string))
				{
					compareTo = long.Parse(compareToThis.ToString());
				}
				else
				{
					compareTo = (long) compareToThis;
				}
			}
			catch
			{
		...
		public PropertyLong(XmlElement element)
		{

			this.m_value = long.Parse(element.InnerText);
		...
		public PropertyLong(XmlReader reader)
		{

			this.m_value = XmlConvert.ToInt64(reader.Value);
		...
		/// </summary>
		public long _Value { get { return this.m_value; } }
